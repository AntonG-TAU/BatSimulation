...
AngleFieldWidth = 2*pi; % omer&anton The Max angle to react to other bats
...
if ObsMemSize > 1 && AnalyzedPulseNum > 1
    ...
    prev_xFinds = [BAT.ObsFindsStruct(minPulse:AnalyzedPulseNum-1).xFinds];
    prev_yFinds = [BAT.ObsFindsStruct(minPulse:AnalyzedPulseNum-1).yFinds];
...
% omer&anton: swarming 'a_' commands
ManueverCmdStruct.a_Total                   = zeros(1, 2, 'single');
ManueverCmdStruct.a_Alignment               = zeros(1, 2, 'single');
ManueverCmdStruct.a_Cohesion                = zeros(1, 2, 'single'); 
ManueverCmdStruct.a_Separation              = zeros(1, 2, 'single');
ManueverCmdStruct.a_Food                    = zeros(1, 2, 'single');
ManueverCmdStruct.a_Obstacle                = zeros(1, 2, 'single'); 
...
switch AllParams.SimParams.TestMode
    ...
    %%%%%%%%%% END FORAGING %%%%%%%%
    case 'swarm_' % omer&anton - we duplicated swarm case below
       %% NEW - swarm
        if NumberOfOtherBats > 0
    ...
    case 'swarm'
       %% omer&anton - modified swarm algorithm
       % calculate and set a_Total by detecting other bats and preys
       % calculate crashes for statistics
       
       sum_CM = zeros(1, 2, 'single'); % center mass
       NumOfNgbrs = single(0); % num of vissble neighbours
       R_2Ngbrs = inf(1, 2, 'single'); % range to neighbours
       a_Alignment = zeros(1, 2, 'single');
       DetectionRange = AllParams.BatSonarParams.BatDetectionRange/xyResolution;
       SeparationRange = 10 * MinDist2OtherBat;
       
       for n = 1:NumberOfOtherBats
           % check if in FoV and range. count num of visible agents:
           BatDist = BatsDistances(n);
           if BatDist <=  DetectionRange
               % if BatsVecAngles(n) <= AngleFieldWidth
                   NumOfNgbrs = NumOfNgbrs + 1;
                   % for a_Alignment calc. weighted by distance
                   WeightedDirection = (DetectionRange - BatDist) * [cos(BatsRelativeDirection(n)) ; sin(BatsRelativeDirection(n))];
                   a_Alignment = a_Alignment + WeightedDirection';
                   % for cohesion calc
                   RelativePos = BatDist*[cos(BatsVecAngles(n)) ; sin(BatsVecAngles(n))];
                   sum_CM = sum_CM + RelativePos';
                   % for separation calc
                   if BatDist <= SeparationRange
                       if any(R_2Ngbrs==inf)
                           R_2Ngbrs = RelativePos';
                       else
                           R_2Ngbrs = R_2Ngbrs + RelativePos';
                       end
                   end
           %    end
           end
       ...
       if NumOfNgbrs >= 1 % 
           ManueverCmdStruct.a_Alignment = a_Alignment / NumOfNgbrs;
           CoM = sum_CM / NumOfNgbrs; % center of mass x,y distance
           R_2Ngbrs_norm  = norm(R_2Ngbrs);
           
           sumC = AllParams.BatFlightParams.C_Alignment + AllParams.BatFlightParams.C_Separation + ...
                  AllParams.BatFlightParams.C_Cohesion + AllParams.BatFlightParams.C_Obstacle;
                     
           ManueverCmdStruct.a_Alignment = AllParams.BatFlightParams.C_Alignment/sumC * ManueverCmdStruct.a_Alignment;
           ManueverCmdStruct.a_Cohesion =  AllParams.BatFlightParams.C_Cohesion/sumC * CoM;
           ManueverCmdStruct.a_Separation = - AllParams.BatFlightParams.C_Separation/sumC * 300 * SeparationRange ./ R_2Ngbrs;
           ManueverCmdStruct.a_Separation = max(min(ManueverCmdStruct.a_Separation, MaxAccel/SampleTime^2),-MaxAccel/SampleTime^2); % clip values from diverging
           ManueverCmdStruct.a_Total = ManueverCmdStruct.a_Alignment + ManueverCmdStruct.a_Cohesion + ...
               ManueverCmdStruct.a_Separation;
                  
       end
       
       % preys
       numPreys = length(PreyFindsStruct.DetectedPreyNum);
       if numPreys > 0
           % choose the closest prey. can also consider the direction
           [PreyDist, Prey] = min(Dist2DetectedPrey);
           DirToPrey = Angle2DetectedPrey(Prey);
           ManueverCmdStruct.a_Food = PreyDist*[cos(DirToPrey) ; sin(DirToPrey)];
           ManueverCmdStruct.a_Total = ManueverCmdStruct.a_Food;
       end
...
%% React to Other BATS
if NumberOfOtherBats > 0 && ~strcmp(AllParams.SimParams.TestMode, 'swarm') %omer&anton - skip if swarm test mode
    %%%% Change 18Aug21- Omer %%%%%
...
switch ManueverCmdStruct.ManueverType
    ...
    otherwise % not 'Hunting' % omer%anton - check if exceeded max or min veloicty
