        ...
        case 'Foraging' % omer&anton - to make sure we always enter this case
            ...
            switch FlightType
                ...
                case 'swarm' % omer&anton - added manually to default params table
                    dt = 0.1 / TimeToManuever; % 0.1 is the control rate in the swarm simulation
                    if RemainTimeToNextMan == TimeToManuever
                        % update to new a_Total
                        % calc norm(a_Total) and check maxAccel condition
                        DtetaNext = randFactor * sqrt(2)*MaxDTetaBat*StdRand*randn; %%% delete this
                    else 
                        DtetaNext = CurrentDteta; % how to calc it
                    end 
                    % calc next pose based on gievn acceleration
                    % set v_x, v_y := CurrentVelocity*cos(TetaB), CurrentVelocity*sin(TetaB)
                    % set a_x, a_y := a_Total(0), a_Total(1);
                    % calc next_v:= [v_x + a_x*dt, v_y + a_y*dt] == [next_v_x, next_v_y]
                    % calc TetaBNext:= arctan(next_v_y/next_v_x)
                    % AccelCommand:=(|next_velocity|-CurrentVelocity)
                otherwise
