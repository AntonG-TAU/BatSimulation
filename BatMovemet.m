...
%             ReqVelocity = max( MaxVelocity * cos(Angle2HuntedPrey + dTetaChase), MinVelocity);
        ...
        case {'ObsMan','FollowWallCarefully'}
        ...
        %%% Foraging %%%
        case 'Foraging' % omer&anton - to make sure we always enter this case
            randFactor = 5;
            if strcmp(AllParams.SimParams.TestMode, 'caveExit')
                 randFactor = 2.5;
            end
            switch FlightType
                ...
                case 'swarm' % omer&anton - added manually to default params table
                    % update to new a_Total
                    a_Total = ManCommandStrct.a_Total;
                    % rotate a_Total by -TetaB
                    R = [cos(-TetaB) sin(-TetaB); ...
                        -sin(-TetaB) cos(-TetaB)];
                    a_Total = R * a_Total';
                    % calc norm(a_Total) and check maxAccel condition
                    if norm(a_Total) > (MaxAccel / SampleTime^2)
                        % a_Total accel value for dt=0.1
                        a_Total = (MaxAccel / SampleTime^2) * (a_Total/norm(a_Total));
                    end
                    V = CurrentVelocity*[cos(TetaB) ; sin(TetaB)];
                    % calc DtetaNext
                    if RemainTimeToNextMan == TimeToManuever
                        % calc Dteta for dt=0.1
                        VNext = V + a_Total * (SampleTime * TimeToManuever);
                        TetaBNext = atan2(VNext(2),VNext(1));
                        DtetaNext = TetaBNext - TetaB;
                        % TetaBNext -> (-pi,pi]
                        DtetaNext = DtetaNext -(2*pi)*floor((DtetaNext+pi)/(2*pi));
                        % increment to dt=0.0005
                        DtetaNext = DtetaNext / TimeToManuever / 2;
                    else
                        DtetaNext = CurrentDteta;
                    end
                    % calc AccelCommand 
                    VNext = V + a_Total * SampleTime;
                    BVelocityNext = min(norm(VNext), NominalVelocity);
                    AccelCommand = BVelocityNext - CurrentVelocity;                          
                otherwise
...
%     BVelocityNext = min(BVelocityNext, NominalVelocity);
    ...
    xBNext = xB + BVelocityNext.*cos(TetaB) ;
    yBNext = yB + BVelocityNext.*sin(TetaB) ;
